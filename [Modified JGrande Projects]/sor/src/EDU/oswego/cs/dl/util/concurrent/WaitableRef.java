...
public class WaitableRef extends SynchronizedRef {
  ...
  public void whenNull(Thread action) throws InterruptedException {
    synchronized (lock_) {
   ...
   * The action is run with the synchronization lock held.
   **/
  public void whenNotNull(Thread action) throws InterruptedException {
    synchronized (lock_) {
      while (value_ == null) lock_.wait();
      if (action != null) action.run();
    }
  ...
  public void whenEqual(Object c, Thread action) throws InterruptedException {
    synchronized (lock_) {
   ...
   * The action is run with the synchronization lock held.
   **/
  public void whenNotEqual(Object c, Thread action) throws InterruptedException {
    synchronized (lock_) {
      while (!(value_ != c)) lock_.wait();
      if (action != null) action.run();
    }
