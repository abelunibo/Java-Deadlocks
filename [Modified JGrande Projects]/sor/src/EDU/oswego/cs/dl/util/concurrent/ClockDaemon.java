 ...
 * that may be used to execute Thread commands in any of three modes --
 ...
 * use as arguments Thread commands that start their own threads
...
public class ClockDaemon extends ThreadFactoryUser  {
  ...
  protected static class TaskNode implements Comparable {
    final Thread command;   // The command to run
    ...
    TaskNode(long w, Thread c, long p) {
      timeToRun_ = w; command = c; period = p;
    }

    TaskNode(long w, Thread c) { this(w, c, -1); }
  }
   ...
   **/
  public Object executeAt(Date date, Thread command) {
   ...
   * 
   *     readerThread = new Thread(new Thread() {
   *      public void run() {
   *        for(;;) {
   ...
   *    timer.executeAfterDelay(60000, new Thread() {
   ...
   * @return taskID -- an opaque reference that can be used to cancel execution request
   **/
  public Object executeAfterDelay(long millisecondsToDelay, Thread command) {
  ...
  /**
   * The runloop is isolated in its own Thread class
   * just so that the main 
   * class need not implement Thread,  which would
   * allow others to directly invoke run, which is not supported.
   **/

  protected class RunLoop extends Thread {
    public void run() {
      try {
        for (;;) {
