...
public class PooledExecutor extends ThreadFactoryUser implements Executor {
   ...
   **/
  protected void addThread(Thread command) {
    ++poolSize_;
  ...
  protected Thread getTask() throws InterruptedException {
    long waitTime = getKeepAliveTime();
    if (waitTime >= 0) 
      return (Thread)(handOff_.poll(waitTime));
    else 
      return (Thread)(handOff_.take());
  }
  ...
  protected class Worker extends Thread {
    protected Thread firstTask_;

    Worker(Thread firstTask) { firstTask_ = firstTask; }

    public void run() {
      try {
        Thread task = firstTask_;
        firstTask_ = null;
        if (task != null)
          task.run();
  ...
  protected abstract class BlockedExecutionHandler {
     ...
     **/
    abstract boolean blockedAction(Thread command);
  }

  /** Class defining Run action **/
  protected class RunWhenBlocked extends BlockedExecutionHandler {
    protected boolean blockedAction(Thread command) {
      command.run();
      return true;
    }
  }
