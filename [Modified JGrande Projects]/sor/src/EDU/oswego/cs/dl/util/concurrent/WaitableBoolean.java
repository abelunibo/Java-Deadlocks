...
public class WaitableBoolean extends SynchronizedBoolean {
  ...
  public  void whenFalse(Thread action) throws InterruptedException {
    synchronized (lock_) {
   ...
   * The action is run with the synchronization lock held.
   **/
  public  void whenTrue(Thread action) throws InterruptedException {
    synchronized (lock_) {
      while (!value_) lock_.wait();
      if (action != null) action.run();
    }
  ...
  public  void whenEqual(boolean c, Thread action) throws InterruptedException {
    synchronized (lock_) {
   ...
   * The action is run with the synchronization lock held.
   **/
  public  void whenNotEqual(boolean c, Thread action) throws InterruptedException {
    synchronized (lock_) {
      while (!(value_ != c)) lock_.wait();
      if (action != null) action.run();
    }
